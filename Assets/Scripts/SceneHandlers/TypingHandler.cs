...
public class TypingHandler : MonoBehaviour
{
        ...
        while (true)
        {
            while (increment < 1f)
            {
                ...
                for (int i = 0; i < typeSpaceCount; i++)
                {
                    if (i == typeSpaceIndex)
                    {
                        typeSpace[i].GetComponent<RectTransform>().localPosition =
                            new Vector3(typeSpace[i].GetComponent<RectTransform>().localPosition.x,
                                typeSpaceY - (2f * flipIncrement(increment, flip)),
                                typeSpace[i].GetComponent<RectTransform>().localPosition.z);
                    }
                    else
                    {
                        typeSpace[i].GetComponent<RectTransform>().localPosition =
                            new Vector3(typeSpace[i].GetComponent<RectTransform>().localPosition.x, typeSpaceY,
                                typeSpace[i].GetComponent<RectTransform>().localPosition.z);
                    }
                }
                yield return null;
    ...
    private IEnumerator animateSelector()
    {
        ...
        while (true)
        {
            while (increment < 1f)
            {
                ...
                if (selectorIndex < 4)
                {
                    currentSelector.GetComponent<RectTransform>().sizeDelta = new Vector2(32, 24);
                }
                else if (selectorIndex < 6)
                {
                    currentSelector.GetComponent<RectTransform>().sizeDelta = new Vector2(39, 24);
                }
                else if (pageIndex == 3)
                {
                    currentSelector.GetComponent<RectTransform>().sizeDelta = new Vector2(146 + (4f * flipIncrement(increment, flip)),
                        14 + (4f * flipIncrement(increment, flip)));
                }
                else
                {
                    currentSelector.GetComponent<RectTransform>().sizeDelta = new Vector2(14 + (4f * flipIncrement(increment, flip)),
                        14 + (4f * flipIncrement(increment, flip)));
                }
                yield return null;
            }
    ...
    private void setSelectorPosition(int newPosition)
    {
        ...
        if (newPosition < 6)
        {
            ...
            currentSelector.GetComponent<RectTransform>().localPosition = new Vector3(selectorPosition.x, selectorPosition.y, 0);
